...
class LIF:
	...
	def show_c(self, show: bool = False, ax: plt.Axes = None, save: bool = False, savename: str = "c.png"):
		ax = sns.heatmap(self.c, ax=ax)
		ax.set_xlabel("Neuron j [-]", fontsize=22)
		ax.set_ylabel("Neuron i [-]", fontsize=22)
		if save:
...
def question_2_exploration(show=False):
	...
	lif = LIF(n_exc=50, n_inh=50, p_EE=0.1, p_EI=0.1, p_IE=0.05, p_II=0.05)
	fig, ax = plt.subplots(figsize=(12, 8))
	ax = lif.show_c(ax=ax)
	fig.savefig("figures/q2a_explo_c.png", dpi=300)
	plt.close(fig)
	lif_output = lif.run(IConst(4.0), IConst(1.0), T_ms=20, dt=0.025)

	n_neurone = 4
	fr_mean = LIF.get_firing_rate(lif_output, reduce=True)
	exc_neuron_indexes = lif.exc_indexes[np.argsort(
		np.abs(fr_mean - LIF.get_firing_rate(lif_output, reduce=False)[lif.exc_indexes])
	)[:n_neurone // 2]]
	inh_neuron_indexes = lif.inh_indexes[np.argsort(
		np.abs(fr_mean - LIF.get_firing_rate(lif_output, reduce=False)[lif.inh_indexes])
	)[:n_neurone - exc_neuron_indexes.size]]
	neuron_indexes = np.concatenate([exc_neuron_indexes, inh_neuron_indexes])
	...
	for i, neuron_idx in enumerate(neuron_indexes):
		...
		# axes[0][i].plot(lif_output.t_space_ms, lif.v_thresh*np.ones_like(lif_output.t_space_ms), color='k')
	...
	axes[1][0].set_ylabel("$g_E$ [$mS/cm^2$]")
	axes[2][0].set_ylabel("$g_I$ [$mS/cm^2$]")
	axes[3][0].set_ylabel(r"I [$\mu A/cm^2$]")
...
def question_2a(resolution: int = 25, show=False):
	...
	firing_rates_lists = []
	for i, ((p_EE_min, p_EE_max), (I_E_min, I_E_max)) in enumerate(zip(p_EE_ranges, I_E_ranges)):
		if i == 0:
			continue
		p_EE_space = np.linspace(p_EE_min, p_EE_max, num=resolution)
		I_E_value_space = np.linspace(I_E_min, I_E_max, num=resolution)
		...
		ax = sns.heatmap(
			...
			xticklabels=['{:,.3f}'.format(x) for x in p_EE_space],
			yticklabels=['{:,.3f}'.format(x) for x in I_E_value_space[::-1]],
			cbar_kws={'label': 'Mean spiking rate [Hz]'},
			ax=ax,
		)
		ax.set_xlabel("$p_{EE}$ [-]")
		ax.set_ylabel(r"$I_E$ [$\mu A/cm^2$]")
		os.makedirs("figures", exist_ok=True)
		plt.savefig(f"figures/q2a_heatmap{i}.png", dpi=300)
		if show:
			plt.show()
		firing_rates_lists.append(firing_rates_list)
	return firing_rates_lists


def question_2b_worker(lif, p_EE, I_E_value):
	lif.set_p(p_EE=p_EE)
	lif_output = lif.run(I_exc_func=IConst(I_E_value), T_ms=100, verbose=False)
	return lif_output


def question_2b(resolution: int = 25, show=False, I_E_values=None, p_EE_values=None):
	# TODO: take the values from question_2a
	# I_E_values = np.array([2.019, 2.019, 2.5, 2.431])
	# p_EE_values = np.array([0.167, 0.17, 0.190, 0.185])
	if I_E_values is None:
		I_E_values = np.array([2.019, 2.019, 2.5])
	if p_EE_values:
		p_EE_values = np.array([0.167, 0.17, 0.190])
	...
	for i, (ax, out) in enumerate(zip(axes, outputs)):
		...
		ax.set_xlabel("Time [ms]")
		ax.get_yaxis().set_visible(False)
		ax.set_title(rf"$I_E = {I_E_values[i]:.3f}$ [$\mu A/cm^2$], " + r"$p_{EE}$" + rf"$ = {p_EE_values[i]:.3f}$ [-]")

	plt.tight_layout(pad=2)
	os.makedirs("figures", exist_ok=True)
	plt.savefig(f"figures/q2b_raster_plots.png", dpi=300)
	if show:
		plt.show()
...
def question_2c_worker(lif, p_EI, p_IE):
	...
	lif_output = lif.run(I_exc_func=IConst(2.019), T_ms=400, verbose=False)
	return LIF.get_firing_rate(lif_output, reduce=True)
...
def question_2c_raster_worker(lif, p_EI, p_IE):
	lif.set_p(p_EI=p_EI, p_IE=p_IE)
	lif_output = lif.run(I_exc_func=IConst(2.019), T_ms=400, verbose=False)
	return lif_output
...
def question_2c_raster(resolution: int = 25, show=False):
	...
	assert p_EI_values.size == p_IE_values.size

	lif = LIF(n_exc=50, n_inh=50, p_EE=0.0, p_EI=0.0, p_IE=0.0, p_II=0.0)
	outputs = apply_func_multiprocess(
		func=question_2c_raster_worker,
	...
	ncols = int(p_EI_values.size / nrows)
	fig, axes = plt.subplots(nrows, ncols, figsize=(14, 10), sharex='all')
	axes = np.ravel([axes])
	for i, (ax, out) in enumerate(zip(axes, outputs)):
		# for n_idx, spikes in enumerate(out.spikes.T):
		...
		ax.get_yaxis().set_visible(False)
		ax.set_title(
			r"$p_{EI}$" + rf"$ = {p_EI_values[i]:.2f}$ [-], " + r"$p_{IE}$" + rf"$ = {p_IE_values[i]:.2f}$ [-]"
		)

	plt.tight_layout(pad=2)
	os.makedirs("figures", exist_ok=True)
	plt.savefig(f"figures/q2c_raster_plots.png", dpi=300)
	if show:
		plt.show()
